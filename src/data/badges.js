...
// ── Beregn hvilke badges brugeren har optjent (ren funktion) ─────────────────
...
export function beregnOpnåedeBadges({ kreationer, gemteAntal, vennerAntal, streak }) {
  ...
  // Tilberedningstid (kun Kok.jsx-kreationer har tidBrugt)
...
export async function synkBadges(brugerId, opnåedeIds) {
  ...
  if (error) return new Set(opnåedeIds)

  const allerede = new Set((existing ?? []).map(r => r.badge_id))
  const nye = opnåedeIds.filter(id => !allerede.has(id))

  if (nye.length) {
    await supabase.from('user_badges').insert(
      nye.map(badge_id => ({ user_id: brugerId, badge_id }))
    )
    nye.forEach(id => allerede.add(id))
  }

  return allerede
}

// ── Hent optjente badges fra Supabase ─────────────────────────────────────────
